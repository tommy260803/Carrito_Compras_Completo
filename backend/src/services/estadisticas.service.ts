...
export const estadisticasService = {
  async tendenciaMensual() {
    ...
    const data = await prisma.$queryRaw<Array<{ mes: string; total: any }>>`
  ...
  async analisisABC() {
    ...
    const data = await prisma.$queryRaw<Array<{ producto_id: number; nombre: string; ingreso: any }>>`
  ...
  async calcularRFM() {
    ...
    const data = await prisma.$queryRaw<Array<{ cliente_id: number; email: string; ordenes: any; gasto: any }>>`
